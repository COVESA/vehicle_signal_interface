...
/*!-----------------------------------------------------------------------
	...
	s h a r e d M e m o r y O p e n
	...
	@return A pointer to the shared memory segement that was opened.
...
sharedMemory_t* sharedMemoryOpen ( void )
{
	...
	if (fd < 0)
	{
		printf ( "Unable to open the shared memory segment[%s] errno: %u[%s].\n",
				 SHARED_MEMORY_SEGMENT_NAME, errno, strerror(errno) );
		return 0;
	}
    //
    ...
    if ( stats.st_size <= 0 )
    {
		...
		//
		sharedMemory = sharedMemoryInitialize ( fd, TOTAL_SHARED_MEMORY_SIZE );
		if ( sharedMemory == 0 )
		{
			printf ( "Unable to initialize the shared memory segment[%s] errno: "
					 "%u[%s].\n", SHARED_MEMORY_SEGMENT_NAME, errno, strerror(errno) );
			(void) close ( fd );
			return 0;
		}
	...
	else
	{
		...
		//
		sharedMemory = mmap ( NULL, sizeof(sharedMemory_t), PROT_READ|PROT_WRITE,
							  MAP_SHARED, fd, 0 );
	...
	printf ( "Shared memory segment has been mapped into memory\n" );
...
/*!-----------------------------------------------------------------------
	...
	v s i S h a r e d M e m o r y C l o s e
	...
	@param[in] sharedMemory - The address of the shared memory segment.
...
/*!-----------------------------------------------------------------------
	...
	s h a r e d M e m o r y I n i t i a l i z e
...
------------------------------------------------------------------------*/
sharedMemory_t* sharedMemoryInitialize ( int fd, size_t sharedMemorySegmentSize )
{
	int             status;
    ...
    if (status != 0)
    {
        printf ( "Unable to resize the shared memory segment to [%lu] bytes - "
                 "errno: %u[%s].\n", TOTAL_SHARED_MEMORY_SIZE, errno, strerror(errno) );
        return 0;
    }
	//
    ...
    if ( status != 0 )
    {
        printf ( "Unable to set shared mutex attribute - errno: %u[%s].\n",
                 status, strerror(status) );
        return 0;
    }
    //
    ...
    if ( status != 0 )
    {
        printf ( "Unable to set recursive mutex attribute - errno: %u[%s].\n",
                 status, strerror(status) );
        return 0;
    }
	//
	...
	for ( int i = 0; i < HASH_BUCKET_COUNT; ++i )
	{
		...
		if ( status != 0 )
		{
			printf ( "Unable to initialize hash bucket mutex - errno: %u[%s].\n",
					 status, strerror(status) );
			return 0;
		}
...
/*!-----------------------------------------------------------------------
	...
	s h a r e d M e m o r y I n s e r t
	...
	@param[in] sharedMemory - The base address of the shared memory segment.
	...
	@return The index into the message list where the message was inserted.
						 ...
						 void* body )
{
	...
	//	key.
	//
	hashBucket_t* hashBucket = sm_getBucketAddress ( sharedMemory, messageHash );
    ...
    //
    //	Return the index of the new message to the caller.
    //
    return newMessage;
}
...
/*!-----------------------------------------------------------------------
	...
	s h a r e d M e m o r y F e t c h
