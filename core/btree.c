                        ...
                        char         key4index )
{
    ...
    //
    //  Compute the minimum and maximum number of records in each node.
    //
    btree->min = btree->minDegree - 1;
    btree->max = ( 2 * btree->minDegree ) - 1;

    //
    //  Initialize the record count and the record key parameters.
    //
    btree->count   = 0;
    btree->numKeys = numberOfKeys;
    btree->keys[0] = key1index;
    btree->keys[1] = key2index;
    btree->keys[2] = key3index;
    btree->keys[3] = key4index;
...
int btree_insert ( btree_t* btree, void* data )
{
    ...
    //
    if ( rootNode->keysInUse >= btree->max )
    {
        bt_node_t* newRoot;
                                      ...
                                      bt_node_t* subtree )
{
    ...
    if ( btree->count > 0 )
    {
        ...
        while ( ! isLeaf ( node ) )
        {
            ...
            //
            node = getRightChild ( btree, node, 0 );
        }
        //
...
void btree_traverse ( btree_t* btree, traverseFunc traverseCB )
{
    ...
    //
    //  If the caller supplied a callback function then go do the traversal,
...
static int btree_compare_function ( btree_t* btree, void* data1, void* data2 )
{
    ...
    //
    for ( unsigned int i = 0; i < btree->numKeys; ++i )
    {
        //
        //  Grab the index into the data structure for the current key number.
        //
        index = btree->keys[i];

        //
        //  Compute the difference between the two pointers at this position.
        //
        status = ptr1[index] - ptr2[index];

        //
        //  If the two pointers are not the same at this index, we are done so
        //  just return the result of the comparison as the value of this
        //  function.
        //
        if ( status != 0 )
        {
    ...
    //
    //  If all the fields that we compared were equal then the two input
    //  pointers have equal keys so return that condition to the caller.  Note
    //  that in this case we will be retuning a zero value.
    //
    return status;
}
...
static void btree_print_function ( btree_t* btree, void* data )
{
    ...
    //
    //  If the system has not yet been completely initialized, the btree
    //  structures may not be completely initialized so skip any print
    ...
    //
    //  For the number of fields in the key for this btree...
    //
    for ( unsigned int i = 0; i < btree->numKeys; ++i )
    {
        //
        //  Print the value of the key at the current index position.
        //
        key = ptr[(unsigned)btree->keys[i]];
...
static void btree_print_header ( btree_t* btree )
{
    ...
    BLOG ( "  Number of Keys: %u\n",  btree->numKeys );
    BLOG ( "        Key List: %u, %u, %u, %u\n",  btree->keys[0],
           btree->keys[1], btree->keys[2], btree->keys[3] );
    BLOG ( "       Root Node: %lx\n", btree->root );
