...
#  ifdef BTREE_VERBOSE
     extern void HexDump ( const char *data, int length, const char *title,
                           int leadingSpaces );
...
//
//  Define the data structures that specify the type and size of the keys of a
//  btree.  The "keyDef" data structure consists of an array of "fieldDef"
//  records and a count of the number of fields in this key.  There can be any
//  number of fields defined for a key and they can be any type in any order.
//


//
//  Define the possible field types that we will support.  Note that all of
//  these are defined in terms of the native types that are defined on a
//  particular platform and should be portable between platforms.
//
//  For example, the type "long" may be defined by the current platform as a
//  32 bit signed value but when moved to another platform, it may be defined
//  as a 64 bit signed value on that platform.
//
typedef enum
{
    ft_invalid = 0,
...
//
//  Define a single field of a key.  Note that the btree record keys will be
//  compared in the order in which the field definitions appear in the
//  "btree_key_def" structure.
//
//  The "type" variable defines the type of field this is (e.g. "int", "long",
//  "string", etc).
//
//  The "offset" is the offset from the beginning of the data structure where
//  this key begins.  This will always be a small integer unless the given
//  data structure is very large.
//
//  The "size" is the size of the field in "type" units.  In other words a
//  field consisting of an array of 3 shorts would be defined as "ft_short,
//  3". A field consisting of a single long would be "ft_long, 1".  An array
//  of 80 characters (possibly a string but not necessarily) would be
//  "ft_char, 80" and a variable length string (null terminated) would be
//  "ft_string, 1".  Note that there is a distinction between an array of
//  "char" and a string here.
//
typedef struct btree_field_def
{
    field_types type;
    int         offset;
    int         size;
...
//
//  Define the key fields for a specific btree.  This structure is a variable
//  sized structure depending on how many fields make up the key for this
//  btree.  The structure has a fixed size header followed by an array of
//  "btree_key_def" structures.  The size of the array is determined by the
//  number of fields that make up the key.
//
//  The user should allocate this structure in the shared memory
//  segment (using "sm_malloc").  The size of the structure can be calculated
//  with something like:
//
//      size = sizeof(struct btree_field_def) * fieldsInKey +
//             sizeof(struct btree_key_def);
//
//  where "fieldsInKey" is the number of fields being defined in this key
//  (i.e. the number of elements in the "btreefields" array being defined).
//
typedef struct btree_key_def
{
    int             fieldCount;
    btree_field_def btreeFields[0];

}   btree_key_def;

//
//  Define a handy macro that will return the size of the whole key definition
//  structure given the number of fields in the key.  The value of this macro
//  can be used in a call to "sm_malloc" to allocate shared memory for the
//  entire key definition structure.
//
#define KEY_DEF_SIZE( numberOfFields )              \
(                                                   \
    ( sizeof ( btree_key_def ) ) +                  \
    ( sizeof ( btree_field_def ) * numberOfFields ) \
)
...
typedef struct btree_t
{
    ...
    offset_t     keyDef;     // The offset to the key definition structure
